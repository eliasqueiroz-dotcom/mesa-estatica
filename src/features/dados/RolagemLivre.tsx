...
export default function RolagemLivre({ ready, rolar }: RolagemLivreProps) {
  ...
  const rolarCombinado = () => {
    ...
    rolar(notacao, (resultados) => {
      ...
      registrarLog('rolagem-livre', `Rolagem livre · ${notacaoTexto} → ${resumo}${resultados.length > 1 ? ` · total ${total}` : ''}`, null);
    });
  };
    ...
    <section className="secao">
      ...
      <div style={{ display: 'flex', gap: '0.4rem', flexWrap: 'wrap', marginBottom: '0.75rem' }}>
        {TODAS_AS_FACES.map((f) => (
          <button key={f} onClick={() => rolarPreset(f)} disabled={!ready}>
            1d{f}
          </button>
        ))}
      </div>
      ...
      <div style={{ display: 'flex', gap: '0.5rem', marginTop: '0.5rem', alignItems: 'center' }}>
        ...
        <button disabled={!ready || rolando} onClick={rolarCombinado}>
          rolar {notacaoTexto}
        </button>
      </div>
