...
- [x] **Fase 5 (investigada a fundo, feita a versão segura)** — a troca completa por `payload.new` (como em `mapaPublicoSync.ts`) foi CONSIDERADA e descartada conscientemente: `npcsSync.ts`/`fichasSync.ts` juntam DUAS tabelas (`*_publico`+`*_privado`) pra montar o objeto, e fazer isso via `payload.new` puro exigiria cachear as duas metades por id, abrindo uma janela real (ainda que pequena) de um cliente ver por um instante um objeto "misturado" (metade nova, metade antiga) — risco que não existe hoje, pra um ganho que já ficou pequeno depois da Fase 4. Conferindo campo a campo, quase não sobrava o que cortar de qualquer forma: as únicas colunas nunca lidas por esses módulos eram `owner_token`/`auth_uid` de `characters_privado` (usadas só pela Edge Function `vincular-jogador`, via `service_role`) — `select('*')` virou `select('dados')`/`select('id, dados')` nesse ponto específico. Impacto honesto: negligível (~70 bytes por linha, dois UUIDs) perto do que as Fases 1-4 economizaram; o peso real que continua sendo buscado por inteiro a cada evento é o `dados` JSON da ficha completa, que não dá pra cortar sem o cache que foi descartado por risco.
  - Achado no caminho (test-only, sem relação com o payload): os testes de guarda de corrida em `fichasSync.test.ts`/`npcsSync.test.ts` tinham um `vi.waitFor(() => booleano)` sem `expect` na última espera — `vi.waitFor` só reage a exceção lançada, não ao valor de retorno, então essa checagem resolvia na primeira tentativa mesmo com o resultado errado (um dos quatro era vazio na prática). Corrigido pra `vi.waitFor(() => expect(...).toBe(true))` nos 4 pontos — a suíte agora verifica de verdade o que já dizia verificar.
...
- [x] **Fase 7 (parcial)** — `npm test`/`tsc`/`eslint` limpos (986 testes) + rodada de navegador nos 3 fluxos + a migração de dados real conferida (log acima). Ficou de fora: teste unitário dedicado de `comprimirImagem.ts` — jsdom não implementa `canvas.getContext('2d')` sem o pacote nativo `canvas` (instalar quebraria "clone limpo" em algumas máquinas, CLAUDE.md); a cobertura real ficou por conta da rodada de navegador.

Migração `0031_storage_imagens_ficha_dono.sql` já aplicada no projeto Supabase real pelo usuário — Fases 1-7 estão fechadas (a Fase 5 na versão segura que sobrou depois de pesar risco x ganho). Item encerrado.
