...
Faltam: corte do `#controle`/forçado pro transporte novo (Fase D ainda atrás da flag desligada no lado do mestre), tela GM-only pra colar o `gm_token` (hoje só a Edge Function `vincular-mestre` existe, sem UI), aba Mapa do jogador (mover o próprio token, ver o tabuleiro).
