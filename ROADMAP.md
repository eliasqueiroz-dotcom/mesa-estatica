...
Faltam: hidratação do `PlayerApp` via Realtime (RLS) no lugar do store local, a própria ficha do jogador editável + rolagem via `resolver-rolagem`, corte do `#controle`/forçado pro transporte novo, tela GM-only pra colar o `gm_token` (hoje só a Edge Function `vincular-mestre` existe, sem UI).
