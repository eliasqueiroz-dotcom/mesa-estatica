...
**Achado — bandeja de dados, resolver no Dia 4**: o tamanho visual do `<div>` da bandeja NÃO reflete o espaço real que a física usa para lançar os dados. Testado empiricamente: aumentar a janela de ~972px para 1280px (com reinicialização completa da bandeja, não só resize ao vivo) não mudou onde os dados caem — sempre no mesmo canto, numa área pequena. Causa: a lib não expõe nenhuma config de "tamanho do chão" (conferido: `id, container, assetPath, theme, themeColor, externalThemes, preloadThemes, scale, delay, enableShadows, shadowTransparency, lightIntensity, offscreen, suspendSimulation` — nada de tray/floor). O `window.resize` listener da lib ajusta só a resolução da câmera/canvas, não a área de física. Com `scale: 16` (vs. padrão 5), o dado ficou grande mas a distância de arremesso não cresceu junto, sobrando espaço vazio visível. **Ação para o Dia 4**: encolher a caixa visual para bater com a área real de jogo, ou reduzir a escala, ou desenhar paredes/bordas visuais na bandeja que comuniquem o espaço real — decidir junto com o resto do acabamento visual dos dados.
