...
**Plano, não implementado ainda.** Decisão confirmada com o usuário: toggle **manual** pros dois estados (desacordado E morto), independente do PV.

Hoje "morto" já é **derivado automaticamente** do PV (`estaMorto(pvAtual, pvMaximo)`, `rules/combate.ts:63-74`, regra do commit `f442350`) e aparece só como texto no modal de inspeção do token (`TokenOverlay.tsx:159-163,266-270`) e no rastreador de combate (`IniciativaPanel.tsx:272-286`) — **não há indicador nenhum no token do mapa em si**.

Plano:
1. Adicionar duas entradas novas em `CONDICOES_COMBATE` (`src/rules/data/condicoesCombate.ts:15-24`): `desacordado` e `morto` — reaproveita 100% o mecanismo existente de toggle (`alternarCondicaoCombate`, `store.ts:1148-1168`), já sincronizado via `sessaoPublicaSync.ts` (confirmado pelo fix do commit `cfdc59c`), já com badge de abreviação (`badgeCondicoes`).
2. Estado final exibido = toggle manual **OU** cálculo automático (`estaMorto(pv)`/`estaForaDeCombate(pv)`) — o mestre pode marcar mesmo quando o PV não bate exatamente com a regra.
3. Propagar os dois flags pro objeto `tokensVisuais` em `MapaTab.tsx:266-277` e `MapaJogadorView.tsx` (mesmo padrão já usado pra `sanidadeCritica`), virar atributos `data-morto`/`data-desacordado` no `<div className="mapa-token">` (`MapaTab.tsx:383-391`).
4. CSS novo em `mapa.css` (mesmo padrão de `data-surto`/`data-turno`, linhas 131-185): grayscale + baixa opacidade pra desacordado, overlay de caveira/X pra morto — visualmente distintos um do outro.
5. Toggle fácil pro mestre: reusar o controle de condições que `IniciativaPanel.tsx` já expõe por combatente + considerar atalho direto no popover do token (`TokenOverlay.tsx`) pra marcar sem precisar abrir a aba Combate.
6. Sem mudança de payload de sync necessária — `condicoesCombate`/`condicaoDuracao` já trafegam pela rede.
